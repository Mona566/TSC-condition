...
def simulate(per_node_result_keys, dirstr):
    ...
    NodesAllTXconfirmedtime = [[] for NodeID in range(NUM_NODES)]
    for mc in range(MONTE_CARLOS):
        ...
        for NodeID in range(NUM_NODES):
            ...
            NodesAllTXconfirmedtime[NodeID]= sorted(Net.Nodes[NodeID].NodesAllTXconfirmedtime)
    ...
    for NodeID in range(NUM_NODES):
        ...
        np.savetxt(dirstr+'/raw/NodesAllTXconfirmedtime'+str(NodeID)+'.csv', 
                   np.asarray(NodesAllTXconfirmedtime[NodeID]), delimiter=',')
...
def plot_results(dirstr, per_node_result_keys):
    ...
    for NodeID in range(NUM_NODES):
        ...
        NodesAllTXconfirmedtime.append([np.loadtxt(dirstr+'/raw/NodesAllTXconfirmedtime'+str(NodeID)+'.csv', delimiter=',')])
    """
    Plot results
    """
    avg_window = 1000
